...
async function main() {
  ...
  for (const rPin of pins) {
    for (const cPin of pins) {
      ...
      const packOrderVariationsToTest = [
        ["U1", "R1", "C1"],
        ["U1", "C1", "R1"],
      ]

      const possibleGeneratedCircuitCodes = []

      for (const packFirstOrder of packOrderVariationsToTest) {
        ...
        possibleGeneratedCircuitCodes.push(
          `export default () => (\n${circuitCode}\n)\n`,
        )
      }

      const { bestCircuitCode } = await selectSchematicWithFewestCrossings(
        possibleGeneratedCircuitCodes,
      )

      const fileName = `pin3set${String(idx).padStart(3, "0")}.circuit.tsx`
      await fs.writeFile(join(outDir, fileName), bestCircuitCode)
      idx++
    }
  }
